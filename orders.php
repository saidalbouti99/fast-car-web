...
<head>
  ...
  <title>My Car Accessories Ordering System : Orders</title>
  <!-- Bootstrap -->
  ...
  <div class="row">
    ...
    <form action="orders.php" method="post" class="form-horizontal">
     <div class="form-group">
          ...
          <div class="col-sm-9">
     <input name="oid"id="orderid" placeholder="Order ID" class="form-control" type="text" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_num']; ?>"required> </div>
        </div>
      <div class="form-group">
          <label for="orderdate" class="col-sm-3 control-label">Date</label>
          <div class="col-sm-9">
      <input name="orderdate"id="orderdate" placeholder="Order Date"class="form-control" type="text" readonly value="<?php if(isset($_GET['edit'])) echo $editrow['fld_order_date']; ?>"required></div>
        </div>
        <div class="form-group">
          <label for="staffid" class="col-sm-3 control-label">Staff</label>
          <div class="col-sm-9">
      <select name="sid"id="staffid"class="form-control" required>
        <?php
      try {
      ...
      <div class="form-group">
          ...
          <div class="col-sm-9">
      <select name="cid"id="customerid"class="form-control"required>
        <?php
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a167552, tbl_staffs_a167552_pt2, tbl_customers_a167552_pt2 WHERE ";
        $sql = $sql."tbl_orders_a167552.fld_staff_num = tbl_staffs_a167552_pt2.fld_staff_num and ";
        $sql = $sql."tbl_orders_a167552.fld_customer_num = tbl_customers_a167552_pt2.fld_customer_num";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a167552");
            $stmt->execute();
            $result = $stmt->fetchAll();
